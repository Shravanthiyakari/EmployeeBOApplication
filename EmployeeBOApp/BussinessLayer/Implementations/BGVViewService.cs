...
namespace EmployeeBOApp.BusinessLayer.Services
{
    public class BGVViewService : IBGVViewService
    {
        ...
        public async Task<(List<TicketingTable>, int, int)> GetPaginatedTicketsAsync(string searchQuery, string requestType, int page, int pageSize)
        {
            return await _repo.GetPaginatedTickets(searchQuery, requestType, page, pageSize);
        }

        public async Task<(bool Success, string Message)> ProcessBGVSubmission(int id, string bgvId, string empId, string empName, string approvedBy)
        {
        ...
        public (byte[] Content, string ContentType, string FileName) ExportTicketsToExcel(string searchQuery)
        {
            var filteredTickets = _repo.GetFilteredTickets(searchQuery);
